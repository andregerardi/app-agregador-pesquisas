...
def resumo_2026(dfx, turno='1t', rejeicao=False):
    ...
    for pref, cfg in CAND_2026.items():
        ...
        ## foto + um card por segmento, tudo na mesma linha
        colunas = st.columns([1] + [1.15] * len(disponiveis))
        ...
        for i, (rotulo, nome_col) in enumerate(disponiveis, start=1):
            valor = ultimo_valor_2026(dfx, nome_col)
            colunas[i].metric(label=rotulo,
...
def grafico_linhas_2026(dfx, sufixo, titulo, y_max=70, col_bra_nulo=None, nota_extra=''):
    ...
    for pref in presentes:
        ...
        fig.add_trace(go.Scatter(
            x=x, y=media, mode='lines+markers', marker=dict(size=2), name=cfg['nome'],
    ...
    if col_bra_nulo and tem_dados_2026(dfx, col_bra_nulo):
        ...
        fig.add_trace(go.Scatter(
            x=x, y=mm_2026(y), mode='lines+markers', marker=dict(size=2), name='Brancos, nulos, NS e NR',
...
def grafico_instituto_2026(dfx, instituto, seg, turno):
    ...
    for pref, cfg in CAND_2026.items():
        ...
        fig.add_trace(go.Scatter(
            ...
            mode='lines+markers', marker=dict(size=2), name=f"{cfg['nome']} - {rotulo.lower()}",
            line=dict(color=cfg['cor'], width=2.5), legendrank=rank))
        if tem_dados_2026(fonte, col_ger):
            fig.add_trace(go.Scatter(
                x=fonte['sigla'], y=pd.to_numeric(fonte[col_ger], errors='coerce'),
                mode='lines+markers', marker=dict(size=2), name=f"{cfg['nome']} - geral",
    ...
    with st.container():
        ...
        with col2:
            if rel == 'Católica':
                ...
                fig = go.Figure()
                ##lula
                fig.add_trace(go.Scatter(x=fonte['sigla'], y=fonte[f'lul_{religi_escolhida}_1t'], mode='lines+markers', marker=dict(size=2), name=f"Lula - {rel}",
                                        line=dict(color='rgba(215, 0, 0, 0.8)', width=2.5),legendrank=1))
                fig.add_trace(go.Scatter(x=fonte['sigla'], y=fonte['lul_ger_1t'],mode='lines+markers', marker=dict(size=2), name=f"Lula - geral", 
                                        line=dict(color='rgba(215, 0, 0, 0.8)', width=1, dash='dot')))
                ##bolsonaro
                fig.add_trace(go.Scatter(x=fonte['sigla'], y=fonte[f'bol_{religi_escolhida}_1t'], mode='lines+markers', marker=dict(size=2), name=f"Bolsonaro - {rel}",
                                        line=dict(color='royalblue', width=2.5),legendrank=2))
                fig.add_trace(go.Scatter(x=fonte['sigla'], y=fonte['bol_ger_1t'],mode='lines+markers', marker=dict(size=2), name=f"Bolsonaro - geral", 
                                        line=dict(color='royalblue', width=1, dash='dot')))
                ##ciro gomes
                fig.add_trace(go.Scatter(x=fonte['sigla'], y=fonte[f'ciro_{religi_escolhida}_1t'], mode='lines+markers', marker=dict(size=2), name=f"Ciro Gomes - {rel}",
                                        line=dict(color='green', width=2.5),legendrank=3))
                fig.add_trace(go.Scatter(x=fonte['sigla'], y=fonte['ciro_ger_1t'],mode='lines+markers', marker=dict(size=2), name=f"Ciro Gomes - geral", 
    ...
    with st.container():
        ...
        with col2:
            if rel2 == 'Católica':
                ...
                fig = go.Figure()
                ##lula
                fig.add_trace(go.Scatter(x=fonte['sigla'], y=fonte[f'lul_{religi_escolhida}_2t'], mode='lines+markers', marker=dict(size=2), name=f"Lula - {rel2}",
                                        line=dict(color='rgba(215, 0, 0, 0.8)', width=2.5),legendrank=1))
                fig.add_trace(go.Scatter(x=fonte['sigla'], y=fonte['lul_ger_2t'],mode='lines+markers', marker=dict(size=2), name=f"Lula - geral", 
                                        line=dict(color='rgba(215, 0, 0, 0.8)', width=1, dash='dot')))
                ##bolsonaro
                fig.add_trace(go.Scatter(x=fonte['sigla'], y=fonte[f'bol_{religi_escolhida}_2t'], mode='lines+markers', marker=dict(size=2), name=f"Bolsonaro - {rel2}",
                                        line=dict(color='royalblue', width=2.5),legendrank=2))
                fig.add_trace(go.Scatter(x=fonte['sigla'], y=fonte['bol_ger_2t'],mode='lines+markers', marker=dict(size=2), name=f"Bolsonaro - geral", 
                                        line=dict(color='royalblue', width=1, dash='dot')))
